    ...
    <div className='bg-[url("./assets/chat-area.jpg")] flex-1 bg-cover bg-center overflow-hidden'>
      <div className='flex flex-col gap-3 overflow-y-auto h-full mx-2 my-1'>
        {userChat.map((message, index) => {
          return (
            <div key={index} className={`px-3 py-1 rounded-md max-w-md shadow-md w-fit flex items-center gap-3 cursor-pointer ${message?.senderId === selectedUser?._id ? "self-end bg-[#d9fcd3]" : "self-start bg-white "}`}>
